...
def hesapla(model,ss, sample_df):
    ...
    travel = model.predict(ss.transform(copy_df))
    ...
    energy = model.predict(ss.transform(copy_df))
    ...
    waste = model.predict(ss.transform(copy_df))
    ...
    diet = model.predict(ss.transform(copy_df))
...
def chart(model, scaler, sample_df, prediction):
    ...
    # Make it a donut chart (white circle in middle)
    centre_circle = plt.Circle((0, 0), 0.60, fc='white')
    fig = plt.gcf()
    fig.gca().add_artist(centre_circle)

    # Title in the center
    plt.text(0, 0, f"{prediction:.0f}\nkgCO₂e",
             ha='center', va='center',
             fontsize=16, fontweight='bold', color="#2E7D32")

    # Save to buffer
    data = io.BytesIO()
    plt.savefig(data, format="png", bbox_inches="tight", transparent=True)
    plt.close()
